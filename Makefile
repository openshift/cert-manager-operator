...
deploy: $(KUSTOMIZE) manifests ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	...
	cd $(PROJECT_ROOT)/config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KUSTOMIZE) build $(PROJECT_ROOT)/config/default | kubectl apply -f -

.PHONY: undeploy
undeploy: $(KUSTOMIZE) ## Undeploy controller from the K8s cluster specified in ~/.kube/config.
	$(KUSTOMIZE) build $(PROJECT_ROOT)/config/default | kubectl delete --ignore-not-found -f -
...
bundle: check-tools $(OPERATOR_SDK_BIN) $(KUSTOMIZE) manifests
	...
	cd $(PROJECT_ROOT)/config/manager && $(KUSTOMIZE) edit set image controller=$(IMG)
	$(KUSTOMIZE) build $(PROJECT_ROOT)/config/manifests | $(OPERATOR_SDK_BIN) generate bundle -q --version $(BUNDLE_VERSION) $(BUNDLE_METADATA_OPTS)

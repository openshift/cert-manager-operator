...
var _ = Describe("HTTP01 Proxy [apigroup:operator.openshift.io]", Label("Platform:Generic"), Ordered, func() {
	...
	Context("on a non-baremetal cluster", func() {
		It("should reject HTTP01Proxy CR with unsupported platform condition", func() {
			...
			_, err := certmanageroperatorclient.OperatorV1alpha1().HTTP01Proxies().Create(ctx, proxy, metav1.CreateOptions{})
			Expect(err).NotTo(HaveOccurred(), "failed to create HTTP01Proxy CR")

			DeferCleanup(func(ctx context.Context) {
				By("deleting HTTP01Proxy CR")
				err := certmanageroperatorclient.OperatorV1alpha1().HTTP01Proxies().Delete(ctx, "cluster", metav1.DeleteOptions{})
				...
				_ = wait.PollUntilContextTimeout(ctx, fastPollInterval, lowTimeout, true, func(ctx context.Context) (bool, error) {
					_, getErr := certmanageroperatorclient.OperatorV1alpha1().HTTP01Proxies().Get(ctx, "cluster", metav1.GetOptions{})
			...
			Eventually(func(g Gomega) {
				fetched, getErr := certmanageroperatorclient.OperatorV1alpha1().HTTP01Proxies().Get(ctx, "cluster", metav1.GetOptions{})

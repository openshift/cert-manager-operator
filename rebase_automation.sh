    ...
    safe_replace_version() {
        ...
        case "$context" in
            ...
            "istio-csr")
                ...
                # Update version in bindata and other generated files (KEY = value on same line)
                sed -i "s/ISTIOCSR_OPERAND_IMAGE_VERSION[[:space:]]*=[[:space:]]*${old_version}/ISTIOCSR_OPERAND_IMAGE_VERSION = ${new_version}/g" "$temp_file"
                # Update YAML format (value on next line, e.g. config/manager/manager.yaml)
                sed -i "s/^\([[:space:]]*value:\)[[:space:]]*${old_version}$/\1 ${new_version}/g" "$temp_file"
                ;;
            "bundle")

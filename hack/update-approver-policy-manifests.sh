...
./bin/yq e 'del(.metadata.labels."helm.sh/chart")' -i ${MANIFESTS_FILE}
./bin/yq e 'del(.spec.template.metadata.labels."helm.sh/chart")' -i ${MANIFESTS_FILE}
...
./bin/yq e '(.. | select(has("app.kubernetes.io/managed-by"))."app.kubernetes.io/managed-by") = "cert-manager-operator"' -i ${MANIFESTS_FILE}
./bin/yq e '(.. | select(has("app.kubernetes.io/name"))."app.kubernetes.io/name") = "cert-manager-approver-policy"' -i ${MANIFESTS_FILE}
./bin/yq e '(.. | select(has("app.kubernetes.io/instance"))."app.kubernetes.io/instance") = "cert-manager-approver-policy"' -i ${MANIFESTS_FILE}
./bin/yq e '(.. | select(has("app"))."app") = "cert-manager-approver-policy"' -i ${MANIFESTS_FILE}

# add app.kubernetes.io/part-of to all labels objects (wherever app.kubernetes.io/name exists)
./bin/yq e '(.. | select(has("app.kubernetes.io/name"))."app.kubernetes.io/part-of") = "cert-manager-operator"' -i ${MANIFESTS_FILE}
...
./bin/yq e '... comments=""' -s '"_output/manifests/" + .kind + "_" + .metadata.name + ".yml" | downcase' ${MANIFESTS_FILE}

...
func (r *Reconciler) createOrApplyValidatingWebhookConfigurations(trustManager *v1alpha1.TrustManager, resourceLabels map[string]string, trustManagerCreateRecon bool) error {
	desired := r.getValidatingWebhookConfigurationObject(resourceLabels)
...
func (r *Reconciler) getValidatingWebhookConfigurationObject(resourceLabels map[string]string) *admissionregistrationv1.ValidatingWebhookConfiguration {
	webhook := decodeValidatingWebhookConfigurationObjBytes(assets.MustAsset(validatingWebhookConfigurationAssetName))

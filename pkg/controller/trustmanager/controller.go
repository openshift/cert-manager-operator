...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Fetch the trustmanagers.openshift.operator.io CR
	...
	if err := r.Get(ctx, req.NamespacedName, trustManager); err != nil {
		if errors.IsNotFound(err) {
			...
			// on deleted requests.
			r.log.V(1).Info("trustmanagers.openshift.operator.io object not found, skipping reconciliation", "request", req)
			return ctrl.Result{}, nil
		}
		return ctrl.Result{}, fmt.Errorf("failed to fetch trustmanagers.openshift.operator.io %q during reconciliation: %w", req.NamespacedName, err)
	}

	if !trustManager.DeletionTimestamp.IsZero() {
		r.log.V(1).Info("trustmanagers.openshift.operator.io is marked for deletion", "name", req.NamespacedName)

		if requeue, err := r.cleanUp(trustManager); err != nil {
			return ctrl.Result{}, fmt.Errorf("clean up failed for %q trustmanagers.openshift.operator.io instance deletion: %w", req.NamespacedName, err)
		} else if requeue {
	...
	// Set finalizers on the trustmanagers.openshift.operator.io resource
	if err := r.addFinalizer(ctx, trustManager); err != nil {
		return ctrl.Result{}, fmt.Errorf("failed to update %q trustmanagers.openshift.operator.io with finalizers: %w", req.NamespacedName, err)
	}
...
func (r *Reconciler) cleanUp(trustManager *v1alpha1.TrustManager) (bool, error) {
	r.eventRecorder.Eventf(trustManager, corev1.EventTypeWarning, "RemoveDeployment", "%s trustmanager marked for deletion, removing all resources created for trust-manager deployment", trustManager.GetName())
	return false, nil
}

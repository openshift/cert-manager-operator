...
// Kubernetes refuses ClusterRole grants the operator does not itself hold. trust-manager's
// operand ClusterRole includes events.k8s.io events create/patch, so the operator must too.

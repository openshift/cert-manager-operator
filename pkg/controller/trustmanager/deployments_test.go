...
func TestDeploymentContainerArgs(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			tmBuilder: testTrustManager().WithDefaultCAPackage(v1alpha1.Enabled),
			expectedArgs: []string{
				fmt.Sprintf("--default-package-location=%s", defaultCAPackageLocation),
			},
		},
		...
		},
		{
			...
			tmBuilder: testTrustManager().WithFilterNonCACerts(v1alpha1.Enabled),
			expectedArgs: []string{
				"--filter-non-ca-certs=true",
			},
		},
		{
			...
			tmBuilder: testTrustManager().WithFilterNonCACerts(v1alpha1.Disabled),
			notExpectedArgs: []string{
				"--filter-non-ca-certs=true",
			},
		},
...
func TestDeploymentDefaultCAPackage(t *testing.T) {
	...
	t.Run("adds arg, volume, mount, and hash annotation when enabled", func(t *testing.T) {
		r := testReconciler(t)
		tm := testTrustManager().WithDefaultCAPackage(v1alpha1.Enabled).Build()

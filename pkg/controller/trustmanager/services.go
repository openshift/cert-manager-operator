...
func (r *Reconciler) createOrApplyServices(trustManager *v1alpha1.TrustManager, resourceLabels map[string]string, trustManagerCreateRecon bool) error {
	service := r.getServiceObject(resourceLabels)
	if err := r.createOrApplyService(trustManager, service, trustManagerCreateRecon); err != nil {
		return err
	}

	metricsService := r.getMetricsServiceObject(resourceLabels)
	if err := r.createOrApplyService(trustManager, metricsService, trustManagerCreateRecon); err != nil {
		return err
	}
	return nil
}

func (r *Reconciler) createOrApplyService(trustManager *v1alpha1.TrustManager, svc *corev1.Service, trustManagerCreateRecon bool) error {
...
func (r *Reconciler) getServiceObject(resourceLabels map[string]string) *corev1.Service {
	...
	updateNamespace(service, operandNamespace)

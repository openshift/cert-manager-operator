...
func (r *Reconciler) disallowMultipleIstioCSRInstances(istiocsr *v1alpha1.IstioCSR, createRecon bool) (err error) {
	...
	defer func() {
		...
		if statusUpdateRequired {
			...
			if errUpdate != nil {
				errUpdate = fmt.Errorf("failed to update %s/%s status: %w", istiocsr.GetNamespace(), istiocsr.GetName(), errUpdate)
	...
	if ignoreProcessing {
		...
		if addProcessingRejectedAnnotation(istiocsr) {
			if err := r.UpdateWithRetry(r.ctx, istiocsr); err != nil {
				return fmt.Errorf("failed to update reject processing annotation to %s/%s: %w", istiocsr.GetNamespace(), istiocsr.GetName(), err)
			}
		}
	}

...
func (r *Reconciler) createOrApplyServices(istiocsr *v1alpha1.IstioCSR, resourceLabels map[string]string) error {
	service := r.getServiceObject(istiocsr, resourceLabels)
	if err := r.createOrApplyService(istiocsr, service); err != nil {
		return err
	}
	...
	if err := r.createOrApplyService(istiocsr, metricsService); err != nil {
		return err
	}
	return nil
}

func (r *Reconciler) createOrApplyService(istiocsr *v1alpha1.IstioCSR, svc *corev1.Service) error {
	return common.ApplyResource(r.ctx, r.CtrlClient, r.log, r.eventRecorder, istiocsr, svc, &corev1.Service{}, fieldOwner,
		func(d, e *corev1.Service) bool { return hasObjectChanged(d, e) },
	)
}

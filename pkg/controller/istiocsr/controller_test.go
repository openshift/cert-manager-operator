...
func TestProcessReconcileRequest(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "validating multiple istiocsr's fails while listing",
			getIstioCSR: func() *v1alpha1.IstioCSR {
				return testIstioCSR()
			},
			preReq: func(r *Reconciler, m *fakes.FakeCtrlClient) {
		...
		{
			name: "validating multiple istiocsr's successful by ignoring latest instance",
			getIstioCSR: func() *v1alpha1.IstioCSR {
				istiocsr := testIstioCSR()
		...
		{
			name: "validating multiple istiocsr's failed to update status",
			getIstioCSR: func() *v1alpha1.IstioCSR {
				istiocsr := testIstioCSR()
				istiocsr.SetNamespace("istiocsr3")
				istiocsr.SetCreationTimestamp(metav1.NewTime(time.Now().Add(time.Second * 2)))
			...
			wantErr: `[multiple instances of istiocsr exists, istiocsr3/istiocsr-test-resource will not be processed: multiple instances of istiocsr exists, istiocsr3/istiocsr-test-resource will not be processed, failed to update istiocsr3/istiocsr-test-resource status: failed to update istiocsr.openshift.operator.io "istiocsr3/istiocsr-test-resource" status: test client error]`,
		},
		{
			name: "validating multiple istiocsr's failed to update annotations",
			getIstioCSR: func() *v1alpha1.IstioCSR {
				istiocsr := testIstioCSR()
				istiocsr.SetNamespace("istiocsr3")
				istiocsr.SetCreationTimestamp(metav1.NewTime(time.Now().Add(time.Second * 2)))

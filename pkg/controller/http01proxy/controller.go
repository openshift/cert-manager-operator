...
// +kubebuilder:rbac:groups=operator.openshift.io,resources=http01proxies,verbs=get;list;watch;update
// +kubebuilder:rbac:groups=operator.openshift.io,resources=http01proxies/status,verbs=get;update
// +kubebuilder:rbac:groups=operator.openshift.io,resources=http01proxies/finalizers,verbs=update
// +kubebuilder:rbac:groups=config.openshift.io,resources=infrastructures,verbs=get;list;watch
// +kubebuilder:rbac:groups=machineconfiguration.openshift.io,resources=machineconfigs,verbs=get;list;watch;create;update;delete
...
func (r *Reconciler) SetupWithManager(mgr ctrl.Manager) error {
	infrastructureMapFunc := func(ctx context.Context, obj client.Object) []reconcile.Request {
		if obj.GetName() != http01proxyObjectName {
...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !proxy.DeletionTimestamp.IsZero() {
		r.log.V(1).Info("http01proxy is marked for deletion", "name", req.Name)
...
func (r *Reconciler) processReconcileRequest(ctx context.Context, proxy *v1alpha1.HTTP01Proxy, req types.NamespacedName) (ctrl.Result, error) {
	if !common.ContainsAnnotation(proxy, controllerProcessedAnnotation) && len(proxy.Status.Conditions) == 0 {
		r.log.V(1).Info("starting reconciliation of newly created http01proxy", "name", proxy.GetName())
	}
	...
	return common.HandleReconcileResult(
		...
		reconcileErr,
		r.log.WithValues("name", proxy.GetName()),
...
func (r *Reconciler) cleanUp(ctx context.Context, proxy *v1alpha1.HTTP01Proxy) error {
	r.log.V(1).Info("cleaning up http01proxy resources", "name", proxy.GetName())
	r.eventRecorder.Eventf(proxy, corev1.EventTypeNormal, "CleanUp", "cleaning up resources for http01proxy %s", proxy.GetName())

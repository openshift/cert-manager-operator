...
func TestMergePodScheduling(t *testing.T) {
	tests := []struct {
		...
		overrideSceduling v1alpha1.CertManagerScheduling
		...
		{
			name: "override scheduling merges with source scheduling",
			sourceScheduling: v1alpha1.CertManagerScheduling{
				NodeSelector: map[string]string{
					"nodeLabel1": "value1",
				},
		...
		{
			name: "override scheduling overrides source scheduling",
			sourceScheduling: v1alpha1.CertManagerScheduling{
				NodeSelector: map[string]string{
					"nodeLabel": "value1",
				},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			actualScheduling := mergePodScheduling(tc.sourceScheduling, tc.overrideSceduling)

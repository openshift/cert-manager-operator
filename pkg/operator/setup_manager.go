...
func NewControllerManager() (*Manager, error) {
	const (
		defaultLoggerName         = "operator-manager"
		defaultLeaderElectionID   = "10c233b5.certmanagers.operator.openshift.io"
		defaultHealthProbeAddress = ":8081"
		defaultHealthCheckPath    = "healthz"
		defaultReadyCheckPath     = "readyz"
	)

	var (
		defaultLeaseDuration = time.Duration(time.Second * 30)
		defaultRenewDuration = time.Duration(time.Second * 20)
	)
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		Logger:                 ctrl.Log.WithName(defaultLoggerName),
		LeaderElection:         false,
		LeaderElectionID:       defaultLeaderElectionID,
		LeaseDuration:          &defaultLeaseDuration,
		RenewDeadline:          &defaultRenewDuration,
		HealthProbeBindAddress: defaultHealthProbeAddress,
	})
	if err != nil {
		return nil, fmt.Errorf("failed to create manager: %w", err)
	}
	if err := mgr.AddHealthzCheck(defaultHealthCheckPath, healthz.Ping); err != nil {
		return nil, fmt.Errorf("failed to set up health check: %w", err)
	}
	if err := mgr.AddReadyzCheck(defaultReadyCheckPath, healthz.Ping); err != nil {
